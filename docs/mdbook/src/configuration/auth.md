...
| `client_id` | string | (required) | OAuth2 client ID registered in kanidm |
...
| `tls_insecure_skip_verify` | bool | `false` | Skip TLS verification for kanidm (dev only) |
...
Each `[[auth.permissions]]` entry maps kanidm groups to OCI scopes:

| Key | Type | Description |
|-----|------|-------------|
| `name` | string | Human-readable name for this rule |
| `groups` | []string | Kanidm groups that grant this permission |
