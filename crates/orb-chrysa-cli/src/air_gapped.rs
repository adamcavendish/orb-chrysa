...
const DEFAULT_NAMESPACE: &str = "orb-chrysa";
const DEFAULT_SERVER_TLS_SECRET: &str = "orb-chrysa-server-tls";
const DEFAULT_RAFT_TLS_SECRET: &str = "orb-chrysa-raft-mtls";
const DEFAULT_STATEFULSET_NAME: &str = "orb-chrysa";
const DEFAULT_HEADLESS_SERVICE: &str = "orb-chrysa-headless";
const DEFAULT_REPLICAS: u16 = 3;
const DEFAULT_OUT: &str = "./orb-chrysa-airgap";
const DEFAULT_IMAGE_REPOSITORY: &str = "ghcr.io/adamcavendish/orb-chrysa-server";
...
pub fn cert_init(args: CertInitArgs) -> Result<()> {
    ...
    let mut ca_params = CertificateParams::new(vec!["orb-chrysa-airgap-ca.local".to_string()])?;
    ca_params.distinguished_name = DistinguishedName::new();
    ca_params.distinguished_name.push(
        DnType::CommonName,
        format!("Orb Chrysa Air-Gapped CA for {registry_host}"),
    );
...
fn server_tls_config(secret_name: &str) -> String {
    format!(
        r#"# Mount Kubernetes Secret `{secret_name}` at /etc/orb-chrysa/tls.
[server.tls]
...
fn raft_tls_config(secret_name: &str) -> String {
    format!(
        r#"# Mount Kubernetes Secret `{secret_name}` at /etc/orb-chrysa/raft-tls.
[raft]
listen = "0.0.0.0:5051"

[raft.tls]
cert_path = "/etc/orb-chrysa/raft-tls/tls.crt"
key_path = "/etc/orb-chrysa/raft-tls/tls.key"
server_ca_path = "/etc/orb-chrysa/raft-tls/server-ca.crt"
client_ca_path = "/etc/orb-chrysa/raft-tls/client-ca.crt"
"#
    )
}
...
fn bundle_readme(
    ...
    format!(
        r#"# Orb Chrysa Air-Gapped Kubernetes Bundle
...
crictl pull {endpoint}/qa/alpine:v1
kubectl run orb-chrysa-pull-test --image={endpoint}/qa/alpine:v1 --restart=Never
```
...
mod tests {
    ...
    fn raft_sans_cover_statefulset_dns() {
        let sans = raft_dns_sans("orb-chrysa", "orb-chrysa-headless", "registry", 2);
        assert!(sans.contains(&"orb-chrysa-0".to_string()));
        assert!(sans.contains(&"orb-chrysa-0.orb-chrysa-headless".to_string()));
        assert!(sans.contains(&"orb-chrysa-0.orb-chrysa-headless.registry.svc".to_string()));
        assert!(
            sans.contains(
                &"orb-chrysa-1.orb-chrysa-headless.registry.svc.cluster.local".to_string()
            )
        );
        assert!(sans.contains(&"*.orb-chrysa-headless".to_string()));
        assert!(sans.contains(&"*.orb-chrysa-headless.registry.svc".to_string()));
        assert!(sans.contains(&"*.orb-chrysa-headless.registry.svc.cluster.local".to_string()));
    }
    ...
    fn helm_values_use_cli_version_and_no_registry_endpoint_value() {
        let yaml = helm_values(
            ...
            "orb-chrysa-server-tls",
            "orb-chrysa-raft-mtls",
    ...
    fn helm_values_keep_explicit_image_override() {
        let yaml = helm_values(
            ...
            "orb-chrysa-server-tls",
            "orb-chrysa-raft-mtls",
            "registry.internal/orb-chrysa-server",
            "v9.9.9",
        );

        assert!(yaml.contains("repository: \"registry.internal/orb-chrysa-server\""));
    ...
    fn bundle_generate_writes_image_overrides() {
        ...
        bundle_generate(BundleGenerateArgs {
            ...
            cert_dir,
            namespace: "orb-chrysa".to_string(),
            server_tls_secret: "orb-chrysa-server-tls".to_string(),
            raft_tls_secret: "orb-chrysa-raft-mtls".to_string(),
            image_repository: "registry.internal/orb-chrysa-server".to_string(),
        ...
        assert!(values.contains("repository: \"registry.internal/orb-chrysa-server\""));
