...
fn main() {
    ...
    let skip = std::env::var("ORB_CHRYSA_SKIP_DASHBOARD").is_ok();
    ...
    println!("cargo:rerun-if-env-changed=ORB_CHRYSA_SKIP_DASHBOARD");

    if skip {
        std::fs::create_dir_all(&dist_dir).ok();
        std::fs::write(
            dist_dir.join("index.html"),
            "<!DOCTYPE html><html><head><title>orb-chrysa</title></head>\
             <body><h1>orb-chrysa</h1><p>Dashboard not built. \
             <code>ORB_CHRYSA_SKIP_DASHBOARD=1</code> was set.</p></body></html>",
        )
        ...
        if std::env::var("PROFILE").as_deref() == Ok("release") {
            panic!(
                "vp (vite-plus) not found. Install it to build the dashboard, \
                 or set ORB_CHRYSA_SKIP_DASHBOARD=1 for a headless build. \
                 See README#dashboard-setup."
            );
        } else {
            println!(
                "cargo:warning=vp (vite-plus) not found — dashboard won't be built. \
                 Set ORB_CHRYSA_SKIP_DASHBOARD=1 to suppress this warning."
            );
            std::fs::create_dir_all(&dist_dir).ok();
            std::fs::write(
                dist_dir.join("index.html"),
                "<!DOCTYPE html><html><head><title>orb-chrysa</title></head>\
                 <body><h1>orb-chrysa</h1><p>Dashboard not built — vp not found.</p></body></html>",
            )
            .expect("failed to write stub index.html");
            return;
        }
