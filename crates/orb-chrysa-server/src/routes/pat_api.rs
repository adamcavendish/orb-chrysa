...
async fn create_token<M: TokenStore, B: BlobStore>(
    ...
    // Generate raw token: "orbchrysa-" + 32 random hex chars
    let mut random_bytes = [0u8; 16];
    rand::rng().fill_bytes(&mut random_bytes);
    let raw_token = format!("orbchrysa-{}", hex::encode(random_bytes));
...
async fn revoke_token<M: TokenStore, B: BlobStore>(
    ...
    if deleted {
        ...
        Err(OrbChrysaError::NameUnknown(format!(
            "token not found: {}",
            id
        )))
    }
...
fn required_identity(
    identity: Option<Extension<crate::auth::token::AuthIdentity>>,
) -> Result<crate::auth::token::AuthIdentity, OrbChrysaError> {
    identity
