...
fn parse_query<T: for<'de> Deserialize<'de>>(uri: &Uri) -> Result<T, OrbChrysaError> {
    Query::<T>::try_from_uri(uri)
        .map(|q| q.0)
        .map_err(|e| OrbChrysaError::NameInvalid(e.to_string()))
}

fn parse_rfc3339_epoch(value: &str, field: &str) -> Result<u64, OrbChrysaError> {
    let timestamp = chrono::DateTime::parse_from_rfc3339(value)
        .map_err(|e| OrbChrysaError::NameInvalid(format!("{} is invalid: {}", field, e)))?
        .timestamp();
    u64::try_from(timestamp)
        .map_err(|_| OrbChrysaError::NameInvalid(format!("{} must be after 1970-01-01", field)))
}
...
async fn repository_dispatch_result<M: ManifestStore, B: BlobStore>(
    ...
    let Some(manifests_pos) = parts.iter().rposition(|part| *part == "manifests") else {
        return Err(OrbChrysaError::NameUnknown(path.to_string()));
    };
    ...
    match (method, tail) {
        ...
        .await
        .map(IntoResponse::into_response),
        _ => Err(OrbChrysaError::Unsupported(
...
async fn get_manifest<M: ManifestStore, B: BlobStore>(
    ...
    let manifest = state
        ...
        .ok_or_else(|| OrbChrysaError::ManifestUnknown(digest.clone()))?;
...
async fn delete_tag<M: ManifestStore, B: BlobStore>(
    ...
    let digest = Digest::from_str_checked(&digest)
        .ok_or_else(|| OrbChrysaError::DigestInvalid(digest.clone()))?;
    if !state.core.metadata.delete_tag(&name, &digest, &tag).await? {
        return Err(OrbChrysaError::NameUnknown(tag));
    }
...
fn dashboard_status_from_raw(raw: membership::ClusterStatus) -> DashboardClusterStatus {
    ...
    DashboardClusterStatus {
        cluster_id: format!("orb-chrysa-{}", raw.node_id),
...
async fn raw_cluster_status(
...
) -> Result<membership::ClusterStatus, OrbChrysaError> {
    ...
    membership::get_status(leader_addr, tls)
        .await
        .map_err(|e| OrbChrysaError::Consensus(e.to_string()))
}
...
async fn cluster_status<M: ManifestStore, B: BlobStore>(
    ...
    let raft = state
        .raft
        .as_ref()
        .ok_or_else(|| OrbChrysaError::Serialization("raft not available".to_string()))?;
