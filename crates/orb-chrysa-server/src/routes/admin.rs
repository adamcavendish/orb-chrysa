...
fn required(value: &str, field: &str) -> Result<(), OrbChrysaError> {
...
fn validate_outbound_proxy(proxy: &mut OutboundProxy) -> Result<(), OrbChrysaError> {
    ...
    match proxy.protocol {
        ...
        OutboundProxyProtocol::Http => {
            let url = proxy.url.as_deref().ok_or_else(|| {
                OrbChrysaError::NameInvalid(
                    "outbound_proxy.url is required for HTTP proxy".to_string(),
                )
            })?;
            ProxyConfig::http(url).map_err(|e| OrbChrysaError::NameInvalid(e.to_string()))?;
            Ok(())
        }
        OutboundProxyProtocol::Socks4 => {
            ...
            ProxyConfig::socks4(url).map_err(|e| OrbChrysaError::NameInvalid(e.to_string()))?;
            Ok(())
        }
        OutboundProxyProtocol::Socks5 => {
            ...
            ProxyConfig::socks5(url).map_err(|e| OrbChrysaError::NameInvalid(e.to_string()))?;
            Ok(())
        }
        OutboundProxyProtocol::Https => Err(OrbChrysaError::NameInvalid(
...
fn validate_strategy(strategy: &MirrorStrategy) -> Result<(), OrbChrysaError> {
    match strategy {
        MirrorStrategy::All => Ok(()),
        MirrorStrategy::Latest { count } if *count > 0 => Ok(()),
        MirrorStrategy::Latest { .. } => Err(OrbChrysaError::NameInvalid(
            "strategy.count must be greater than zero".to_string(),
        )),
        MirrorStrategy::Pattern { pattern } if !pattern.trim().is_empty() => Ok(()),
        MirrorStrategy::Pattern { .. } => Err(OrbChrysaError::NameInvalid(
            "strategy.pattern is required".to_string(),
        )),
    }
}

fn validate_warm_filters(filters: &[WarmFilter]) -> Result<(), OrbChrysaError> {
...
fn validate_mirror_rule(rule: &mut MirrorRule) -> Result<(), OrbChrysaError> {
    required(&rule.id, "id")?;
...
fn validate_proxy_cache(cache: &mut ProxyCache) -> Result<(), OrbChrysaError> {
...
async fn get_rule<M: AdminStore, B: BlobStore>(
    ...
    match state.core.metadata.get_mirror_rule(&id).await? {
        ...
        None => Err(OrbChrysaError::NameUnknown(id)),
    }
}
...
async fn trigger_job<M: AdminStore, B: BlobStore>(
    ...
    if state.core.metadata.trigger_sync_job(&id).await? {
        ...
        Err(OrbChrysaError::NameUnknown(id))
    }
}
...
async fn list_helm_versions<M: AdminStore, B: BlobStore>(
    ...
    match state.core.metadata.list_helm_chart_versions(&name).await? {
        ...
        None => Err(OrbChrysaError::NameUnknown(name)),
    }
}
