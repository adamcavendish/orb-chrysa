...
async fn metrics<M: MetadataStore, B: BlobStore>(
    ...
    let mut body = String::from(
        "# HELP orb_chrysa_up Registry process health.\n\
         # TYPE orb_chrysa_up gauge\n\
         orb_chrysa_up 1\n",
    );
    ...
    if let Some(raft) = state.raft.as_ref() {
        ...
        body.push_str(&format!(
            "# HELP orb_chrysa_raft_leader 1 if this node is the Raft leader.\n\
             # TYPE orb_chrysa_raft_leader gauge\n\
             orb_chrysa_raft_leader {}\n\
             # HELP orb_chrysa_raft_quorum Required healthy voters for quorum.\n\
             # TYPE orb_chrysa_raft_quorum gauge\n\
             orb_chrysa_raft_quorum {}\n\
             # HELP orb_chrysa_raft_healthy_voters Voters caught up with the leader.\n\
             # TYPE orb_chrysa_raft_healthy_voters gauge\n\
             orb_chrysa_raft_healthy_voters {}\n",
    ...
    body.push_str(&format!(
        "# HELP orb_chrysa_gc_last_run_timestamp_seconds Last GC sweep start time.\n\
         # TYPE orb_chrysa_gc_last_run_timestamp_seconds gauge\n\
         orb_chrysa_gc_last_run_timestamp_seconds {}\n\
         # HELP orb_chrysa_gc_last_deleted_blobs Blobs deleted by the last GC sweep.\n\
         # TYPE orb_chrysa_gc_last_deleted_blobs gauge\n\
         orb_chrysa_gc_last_deleted_blobs {}\n\
         # HELP orb_chrysa_gc_last_delete_errors Blob delete errors from the last GC sweep.\n\
         # TYPE orb_chrysa_gc_last_delete_errors gauge\n\
         orb_chrysa_gc_last_delete_errors {}\n",
    ...
    if let Some(auth) = state.auth.as_ref() {
        ...
        body.push_str(&format!(
            "# HELP orb_chrysa_auth_jwks_keys Cached OIDC JWKS verification keys.\n\
             # TYPE orb_chrysa_auth_jwks_keys gauge\n\
             orb_chrysa_auth_jwks_keys {}\n\
             # HELP orb_chrysa_auth_jwks_cache_age_seconds Age of the cached JWKS material.\n\
             # TYPE orb_chrysa_auth_jwks_cache_age_seconds gauge\n\
             orb_chrysa_auth_jwks_cache_age_seconds {}\n\
             # HELP orb_chrysa_auth_jwks_stale_cache 1 when serving from S3 last-good JWKS cache.\n\
             # TYPE orb_chrysa_auth_jwks_stale_cache gauge\n\
             orb_chrysa_auth_jwks_stale_cache {}\n\
             # HELP orb_chrysa_auth_jwks_refresh_failures_total JWKS refresh failure count.\n\
             # TYPE orb_chrysa_auth_jwks_refresh_failures_total counter\n\
             orb_chrysa_auth_jwks_refresh_failures_total {}\n",
            metrics.key_count,
        ...
        if let Some(endpoint) = metrics.endpoint {
            body.push_str(&format!(
                "# HELP orb_chrysa_auth_jwks_endpoint_info Active JWKS source endpoint.\n\
                 # TYPE orb_chrysa_auth_jwks_endpoint_info gauge\n\
                 orb_chrysa_auth_jwks_endpoint_info{{endpoint=\"{}\"}} 1\n",
...
async fn server_raft_status<M: MetadataStore, B: BlobStore>(
    State(state): State<Arc<AppState<M, B>>>,
) -> Result<impl IntoResponse, crate::error::OrbChrysaError> {
    let raft = state.raft.as_ref().ok_or_else(|| {
        crate::error::OrbChrysaError::Serialization("raft not available".to_string())
    })?;
...
/// Build a `HeaderValue` from a string, mapping errors to `OrbChrysaError::Serialization`.
...
mod tests {
    ...
    async fn metrics_endpoint_returns_process_health_metric() {
        ...
        assert!(text.contains("orb_chrysa_up 1"));
    }
