...
fn unknown_upload(session_id: &str) -> OrbChrysaError {
    OrbChrysaError::BlobUploadUnknown(session_id.to_string())
}
...
pub async fn dispatch_start<M: ManifestStore, B: BlobStore>(
    ...
    if *method != Method::POST {
        return Err(OrbChrysaError::Unsupported("method not allowed".into()));
    }
    let _permit = state
        .core
        .upload_semaphore
        .try_acquire()
        .map_err(|_| OrbChrysaError::TooManyRequests("upload limit reached".into()))?;
...
async fn start_upload<M: ManifestStore, B: BlobStore>(
    ...
    if let Some(digest_str) = query.get("digest") {
        let digest = Digest::from_str_checked(digest_str)
            .ok_or_else(|| OrbChrysaError::DigestInvalid(digest_str.clone()))?;
        let body = axum::body::to_bytes(req.into_body(), 1024 * 1024 * 1024)
            .await
            .map_err(|e| OrbChrysaError::BlobUploadInvalid(e.to_string()))?;
        ...
        state
            ...
            .await
            .map_err(|e| OrbChrysaError::DigestInvalid(e.to_string()))?;
        state
            .core
            .metadata
    ...
    if let (Some(mount_digest), Some(from_repo)) = (query.get("mount"), query.get("from")) {
        let digest = Digest::from_str_checked(mount_digest)
            .ok_or_else(|| OrbChrysaError::DigestInvalid(mount_digest.clone()))?;
...
async fn complete_upload<M: ManifestStore, B: BlobStore>(
    ...
    let digest_str = query
        .get("digest")
        .ok_or_else(|| OrbChrysaError::DigestInvalid("missing digest query param".into()))?
        .clone();
    let digest = Digest::from_str_checked(&digest_str)
        .ok_or_else(|| OrbChrysaError::DigestInvalid(digest_str.clone()))?;
