...
impl AuthService {
    pub async fn new(
        ...
        let first_signing_key = config.token_signing_keys.first().ok_or_else(|| {
            OrbChrysaError::Internal("at least one token signing key is required".to_string())
        })?;
        let signing_key_bytes = base64::Engine::decode(
            &base64::engine::general_purpose::STANDARD,
            first_signing_key,
        )
        .map_err(|e| OrbChrysaError::Internal(format!("invalid token signing key: {}", e)))?;
        ...
        .map_err(|e| OrbChrysaError::Internal(format!("invalid session encryption key: {}", e)))?;

        let session_key: [u8; 32] = session_key_bytes.try_into().map_err(|_| {
            OrbChrysaError::Internal("session encryption key must be 32 bytes".to_string())
        })?;
    ...
    async fn fetch_fresh_auth_material(
        config: &AuthConfig,
    ) -> Result<FreshAuthMaterial, OrbChrysaError> {
        ...
        Err(OrbChrysaError::Internal(format!(
    ...
    async fn load_cached_material(
        ...
        jwks_s3_cache: Option<&JwksS3Cache>,
        config: &AuthConfig,
        fetch_err: OrbChrysaError,
    ) -> Result<OidcDiscovery, OrbChrysaError> {
        {
            let mut cache = jwks_cache.write().await;
    ...
    pub async fn refresh_jwks_once(&self) -> Result<(), OrbChrysaError> {
    ...
    pub async fn validate_token<M: TokenStore>(
        ...
        token: &str,
        metadata: &M,
    ) -> Result<AuthIdentity, OrbChrysaError> {
        // 1. Try as orb-chrysa PAT (starts with "orbchrysa-")
        if token.starts_with("orbchrysa-") {
            return self.validate_pat(token, metadata).await;
        }

        // 2. Try as orb-chrysa OCI bearer token (signed by us)
    ...
    fn validate_oci_token(&self, token: &str) -> Result<AuthIdentity, OrbChrysaError> {
        let token_data = jsonwebtoken::decode::<token::TokenClaims>(
            token,
            &self.token_verification_key,
            &jsonwebtoken::Validation::default(),
        )
        .map_err(|_| OrbChrysaError::Unauthorized {
            message: "invalid token".to_string(),
            realm: None,
            service: None,
            scope: None,
    ...
    async fn validate_kanidm_token(&self, token: &str) -> Result<AuthIdentity, OrbChrysaError> {
        ...
        let kid = header.kid.ok_or_else(|| OrbChrysaError::Unauthorized {
    ...
    pub fn check_admin_access(&self, identity: &AuthIdentity) -> Result<(), OrbChrysaError> {
    ...
    pub fn mint_oci_token(
        ...
        jsonwebtoken::encode(&header, &claims, &self.token_signing_key)
            .map_err(|e| OrbChrysaError::Internal(format!("failed to mint token: {}", e)))
    }
...
mod tests {
    ...
    fn auth_config() -> AuthConfig {
        AuthConfig {
            issuer_url: "https://idp.example.test/oauth2/openid/orb-chrysa".to_string(),
    ...
    fn discovery_doc() -> serde_json::Value {
        serde_json::json!({
            ...
            "jwks_uri": "https://idp.example.test/oauth2/openid/orb-chrysa/public_key.jwk",
