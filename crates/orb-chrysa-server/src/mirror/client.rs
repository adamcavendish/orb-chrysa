...
fn parse_bearer_challenge(header: &str) -> Result<BearerChallenge, OrbChrysaError> {
    ...
    Ok(BearerChallenge {
        realm: realm
            .ok_or_else(|| OrbChrysaError::Upstream("no realm in Bearer challenge".into()))?,
        service,
    })
}
...
impl UpstreamClient {
    ...
    fn build_http_client(
        ...
        insecure_tls: bool,
    ) -> Result<TokioClient, OrbChrysaError> {
        let mut builder = TokioClient::builder()
            .user_agent("orb-chrysa/0.1")
            ...
            && proxy.protocol != OutboundProxyProtocol::None
        {
            let Some(url) = proxy.url.as_deref().filter(|v| !v.trim().is_empty()) else {
                return Err(OrbChrysaError::NameInvalid(
            ...
            let mut config = match proxy.protocol {
                ...
                OutboundProxyProtocol::Http => ProxyConfig::http(url)
                    .map_err(|e| OrbChrysaError::NameInvalid(e.to_string()))?,
                ...
                OutboundProxyProtocol::Https => {
                    return Err(OrbChrysaError::Unsupported(
        ...
        builder
            .build()
            .map_err(|e| OrbChrysaError::Upstream(format!("HTTP client: {}", e)))
    }

    async fn http(&self, upstream: &UpstreamRef) -> Result<TokioClient, OrbChrysaError> {
    ...
    async fn ensure_auth_scope(
        ...
        let mut req = http
            .get(&url)
            .map_err(|e| OrbChrysaError::Upstream(format!("auth probe: {}", e)))?;
        ...
        let www_auth = resp
            ...
            .ok_or_else(|| {
                OrbChrysaError::Upstream(format!(
    ...
    pub async fn ensure_auth(&self, upstream: &UpstreamRef) -> Result<(), OrbChrysaError> {
        self.ensure_auth_scope(upstream, UpstreamScope::Pull).await
    }

    pub async fn ensure_push_auth(&self, upstream: &UpstreamRef) -> Result<(), OrbChrysaError> {
    ...
    async fn fetch_token(
        ...
        let mut req = http
            .get(&url)
            .map_err(|e| OrbChrysaError::Upstream(format!("token fetch: {}", e)))?;
        if let (Some(u), Some(p)) = (&upstream.username, &upstream.password) {
            req = req.basic_auth(u, Some(p));
        }
        ...
        let body: serde_json::Value = resp
            .json()
            .await
            .map_err(|e| OrbChrysaError::Upstream(format!("token response: {}", e)))?;

        body.get("token")
            .or_else(|| body.get("access_token"))
            .and_then(|v| v.as_str())
            .map(|s| s.to_string())
            .ok_or_else(|| OrbChrysaError::Upstream("no token in auth response".into()))
    }
    ...
    async fn retry(
        ...
        url: &str,
        accept: Option<&str>,
    ) -> Result<aioduct::Response, OrbChrysaError> {
    ...
    async fn retry_with_body(
        ...
        let mut last_err: Option<OrbChrysaError> = None;
        ...
        for attempt in 0..=MAX_RETRIES {
            ...
            let builder = http
                ...
                .map_err(|e| OrbChrysaError::Upstream(e.to_string()))?;
            ...
            let resp = match builder.send().await {
                ...
                Err(e) => {
                    if is_retryable_err(&e) && attempt < MAX_RETRIES {
                        ...
                        last_err = Some(OrbChrysaError::Upstream(e.to_string()));
                        continue;
                    }
                    return Err(OrbChrysaError::Upstream(e.to_string()));
                }
            };
            ...
            if resp.status() == StatusCode::UNAUTHORIZED && attempt < MAX_RETRIES {
                ...
                last_err = Some(OrbChrysaError::Upstream("401 Unauthorized".into()));
                continue;
            }
            ...
            if is_retryable(resp.status()) && attempt < MAX_RETRIES {
                ...
                last_err = Some(OrbChrysaError::Upstream(format!("HTTP {}", resp.status())));
                continue;
            }
        ...
        Err(last_err.unwrap_or_else(|| OrbChrysaError::Upstream("max retries exceeded".into())))
    }
    ...
    pub async fn head_manifest(
        ...
        upstream: &UpstreamRef,
        reference: &str,
    ) -> Result<Option<(String, String)>, OrbChrysaError> {
        let url = format!(
            "{}/v2/{}/manifests/{}",
            upstream.base_url(),
            upstream.repository,
        ...
        match resp.status() {
            ...
            s => Err(OrbChrysaError::Upstream(format!(
    ...
    pub async fn get_manifest(
        ...
        upstream: &UpstreamRef,
        reference: &str,
    ) -> Result<ManifestData, OrbChrysaError> {
        let url = format!(
            "{}/v2/{}/manifests/{}",
            upstream.base_url(),
            upstream.repository,
        ...
        let resp = self
            ...
            .await?
            .error_for_status()
            .map_err(|e| OrbChrysaError::Upstream(format!("GET manifest {}: {}", reference, e)))?;
        ...
        let body = resp
            .bytes()
            .await
            .map_err(|e| OrbChrysaError::Upstream(format!("GET manifest body: {}", e)))?
            .to_vec();
    ...
    pub async fn get_blob(
        ...
        self.retry(upstream, http::Method::GET, &url, None)
            .await?
            .error_for_status()
            .map_err(|e| OrbChrysaError::Upstream(format!("GET blob {}: {}", digest, e)))
    }
    ...
    pub async fn push_blob_stream(
        ...
        if !matches!(
            ...
            return Err(OrbChrysaError::Upstream(format!(
        ...
        let upload_location = absolute_location(
            ...
            start_resp
                ...
                .and_then(|v| v.to_str().ok())
                .ok_or_else(|| {
                    OrbChrysaError::Upstream("blob upload response missing Location".into())
                })?,
        );
        ...
        let builder = http
            .put(&upload_url)
            .map_err(|e| OrbChrysaError::Upstream(format!("PUT blob upload: {}", e)))?
        ...
        let resp = builder
            .send()
            .await
            .map_err(|e| OrbChrysaError::Upstream(format!("PUT blob upload {}: {}", digest, e)))?;
    ...
    pub async fn list_tags(&self, upstream: &UpstreamRef) -> Result<Vec<String>, OrbChrysaError> {
        ...
        loop {
            ...
            let resp = self
                ...
                .await?
                .error_for_status()
                .map_err(|e| OrbChrysaError::Upstream(format!("GET tags list: {}", e)))?;
            let body: TagsListResponse = resp
                .json()
                .await
                .map_err(|e| OrbChrysaError::Upstream(format!("tags list response: {}", e)))?;
