...
pub enum ConfigError {
    ...
    HostnameNotSet,
    #[error("HOSTNAME '{0}' must contain a dash followed by a number (e.g. orb-chrysa-0)")]
...
/// Returns ordinal + 1 (so orb-chrysa-0 → node_id 1).
...
/// ORB_CHRYSA_RAFT_ADVERTISE_HOST can override the host portion when pods need
/// to advertise a DNS name that differs from HOSTNAME.
/// e.g. HOSTNAME=orb-chrysa-0 + port 5051 -> "orb-chrysa-0:5051"
pub fn resolve_advertise_addr(listen: &str) -> Result<String, ConfigError> {
    let hostname = std::env::var("ORB_CHRYSA_RAFT_ADVERTISE_HOST")
...
impl Config {
    ...
    pub fn default_dev() -> Self {
        if std::env::var("HOSTNAME").is_err() {
            ...
            unsafe { std::env::set_var("HOSTNAME", "orb-chrysa-0") };
        }
        Config {
            server: ServerConfig {
...
mod tests {
    ...
    fn base_config() -> Config {
        Config {
            ...
            raft: RaftConfig {
                listen: default_raft_listen(),
                data_dir: default_data_dir(),
                discovery_dns: "orb-chrysa".to_string(),
                tls: None,
                kubernetes: RaftKubernetesConfig::default(),
            },
            gc: GcConfig::default(),
    ...
    fn kubernetes_reconcile_validates_required_fields() {
        ...
        config.raft.kubernetes.namespace = "orb-chrysa".to_string();
        config.raft.kubernetes.statefulset_name = "orb-chrysa".to_string();
        assert!(config.validate().is_ok());
