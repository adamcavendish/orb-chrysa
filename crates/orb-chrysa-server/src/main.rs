...
async fn main() -> Result<(), AppError> {
    let env_filter = EnvFilter::from_default_env().add_directive("orb_chrysa_server=info".parse()?);
    match std::env::var("ORB_CHRYSA_LOG_FORMAT").as_deref() {
    ...
    if let Some(ref tls_cfg) = server_tls_config {
        ...
        tracing::info!("orb-chrysa HTTPS listening on {}", listen);
        ...
        tracing::info!("orb-chrysa listening on {}", listen);
