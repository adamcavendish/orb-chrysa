...
impl RaftMetadataStore {
    ...
    async fn write(&self, req: Request) -> Result<Response, OrbChrysaError> {
        match self.raft.client_write(req.clone()).await {
            ...
            Err(openraft::error::RaftError::APIError(ClientWriteError::ForwardToLeader(fwd))) => {
                ...
                network::forward_client_write(&leader_addr, self.tls.clone(), &req)
                    .await
                    .map_err(OrbChrysaError::Consensus)
            }
            Err(e) => Err(OrbChrysaError::Consensus(format!("{}", e))),
        }
    }
    ...
    async fn write_manifest(
        &self,
        req: ManifestRequest,
    ) -> Result<ManifestResponse, OrbChrysaError> {
        match self.write(Request::Manifest(req)).await? {
            Response::Manifest(r) => Ok(r),
            other => Err(OrbChrysaError::Internal(format!(
    ...
    ) -> Result<MirrorConfigResponse, OrbChrysaError> {
    ...
    async fn write_job(&self, req: JobRequest) -> Result<JobResponse, OrbChrysaError> {
    ...
    async fn write_token(&self, req: TokenRequest) -> Result<TokenResponse, OrbChrysaError> {
...
impl ManifestStore for RaftMetadataStore {
    ...
    async fn delete_tag(
        ...
        match resp {
            ...
            | ManifestResponse::BlobDeleteStatus(_) => Err(OrbChrysaError::Internal(
    ...
    async fn record_blob_delete_request(
        ...
        match resp {
            ...
            ManifestResponse::Ok
            | ManifestResponse::Bool(_)
            | ManifestResponse::DeleteCounts(_) => Err(OrbChrysaError::Internal(
...
impl MirrorConfigStore for RaftMetadataStore {
    ...
    async fn trigger_mirror_rule(&self, id: &str) -> Result<Option<SyncJob>, OrbChrysaError> {
        ...
        match resp {
            ...
            MirrorConfigResponse::Bool(false) => Err(OrbChrysaError::Conflict(
...
impl TokenStore for RaftMetadataStore {
    ...
    async fn delete_personal_access_token(
        ...
        match resp {
            ...
            TokenResponse::Ok => Err(OrbChrysaError::Internal(
