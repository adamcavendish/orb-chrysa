...
export default function KubernetesSetup() {
  ...
  const [namespace, setNamespace] = createSignal("orb-chrysa");
  const [serverSecret, setServerSecret] = createSignal("orb-chrysa-server-tls");
  const [raftSecret, setRaftSecret] = createSignal("orb-chrysa-raft-mtls");
  ...
  const snippets = createMemo<Snippet[]>(() => {
    ...
    const ns = namespace().trim() || "orb-chrysa";
    const serverTlsSecret = serverSecret().trim() || "orb-chrysa-server-tls";
    const raftTlsSecret = raftSecret().trim() || "orb-chrysa-raft-mtls";

    return [
      {
        title: t("setup.snippet.cert"),
        note: t("setup.snippet.certNote"),
        body: `orb-chrysa-cli air-gapped cert init \\
  --registry-host ${host} \\
  --namespace ${ns} \\
  ...
  --out ./orb-chrysa-airgap`,
      },
      {
        ...
        body: `orb-chrysa-cli air-gapped k8s bundle-generate \\
  ...
  --out ./orb-chrysa-airgap`,
      },
      {
        ...
        body: `helm upgrade --install orb-chrysa ./charts/orb-chrysa \\
  --namespace ${ns} \\
  --create-namespace \\
...
install -m 0644 ./orb-chrysa-airgap/containerd/ca.crt /etc/containerd/certs.d/${ep}/ca.crt
install -m 0644 ./orb-chrysa-airgap/containerd/hosts.toml /etc/containerd/certs.d/${ep}/hosts.toml
systemctl restart containerd`,
      },
      {
        title: t("setup.snippet.pullSecret"),
        note: t("setup.snippet.pullSecretNote"),
        body: `kubectl -n ${ns} create secret docker-registry orb-chrysa-pull \\
  --docker-server=${ep} \\
  ...
  --docker-password=TOKEN`,
      },
      {
        ...
        body: `curl --cacert ./orb-chrysa-airgap/certs/ca.crt https://${ep}/v2/
crictl pull ${ep}/qa/alpine:v1
kubectl run orb-chrysa-pull-test --image=${ep}/qa/alpine:v1 --restart=Never`,
      },
    ];
  });
